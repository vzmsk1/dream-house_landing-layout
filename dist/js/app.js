    ...
    (() => {
            ...
            if (spoilersArray.length > 0) {
                ...
                if (spoilersClose.length) document.addEventListener("click", (function(e) {
                    ...
                    if (!el.closest("[data-spoilers]")) spoilersClose.forEach((spoilerClose => {
                        ...
                        const spoilerSpeed = spollersBlock.dataset.spoilersSpeed ? parseInt(spoilersBlock.dataset.spoilersSpeed) : 500;
                ...
                document.addEventListener("click", function(e) {
                    ...
                    if (buttonOpen) {
                        ...
                        if (this._dataValue !== "error") {
                            ...
                            this.open();
                            return;
                        } else this.popupLogging(`${buttonOpen.classList} is have an empty atribute`);
                        return;
                    }
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    if (!formRequiredItem.value.trim()) {
                        ...
                        if (!phoneRegex.test(formRequiredItem.value)) {
                            formRequiredItem.dataset.error = "please enter a valid phone number";
                            this.addError(formRequiredItem);
                            error++;
                        } else {
        ...
        class RangeSlider {
            ...
            initRange(range) {
                ...
                if (this.output) {
                    ...
                    if (isMulti) this.wrapper.insertBefore(this.output, range); else {
                ...
                if (range.dataset.modifier) this.wrapper.classList.add(range.dataset.modifier);
                ...
                if (circular) {
                    ...
                    this.output.addEventListener("pointerdown", (() => this.output.addEventListener("pointermove", pointerMove)));
                    this.output.addEventListener("pointerup", (() => this.output.removeEventListener("pointermove", pointerMove)));
                    this.updateCircle();
                } else range.addEventListener("input", (() => this.updateRange()));
            ...
            updateRange() {
                if (this.lower) if (this.lower.valueAsNumber > this.range.valueAsNumber) {
                    this.range.value = this.lower.valueAsNumber;
                    return;
                }
                if (this.upper) if (this.upper.valueAsNumber < this.range.valueAsNumber) {
                ...
                if (this.lower) this.lower.style.setProperty(this.settings.varPercentUpper, `${value}%`);
                if (this.output) {
        ...
        function stringToType(obj) {
            ...
            Object.keys(object).forEach((key => {
                if (typeof object[key] === "string" && object[key].charAt(0) === ":") object[key] = JSON.parse(object[key].slice(1));
            }));
            return object;
        }
        function uuid() {
        ...
        const round_slider_elements = document.querySelectorAll("[data-range]");
        round_slider_elements.forEach((element => {
        ...
        function makesvg(percentage, inner_text = "") {
            ...
            if (inner_text) svg += '<text class="circle-chart__subline" x="16.91549431" y="22">' + inner_text + "</text>";
            svg += " </g></svg>";
            return svg;
        }
        (function($) {
        ...
        const initSliders = () => {
            if (document.querySelector(".steps__slider")) new core(".steps__slider", {
                ...
                on: {
                    init: function() {
                        document.querySelector(".steps__fraction").textContent = `${this.activeIndex + 1}`;
                    },
                    slideChange: function() {
                        const circlechart = document.querySelector(".steps__circlechart");
                        const circlechartClone = circlechart.cloneNode(true);
                        const activeIndex = this.activeIndex;
                        const previousIndex = this.previousIndex;
                        const updateBullets = () => {
                            const bullets = document.querySelectorAll(".swiper-pagination-bullet");
                            bullets.forEach(((bullet, index) => {
                                if (index <= activeIndex || circlechartClone.dataset.percentage === "100") bullet.classList.add("_active"); else if (index > activeIndex || circlechartClone.dataset.percentage === "0") bullet.classList.remove("_active");
                            }));
                        };
                        const updateCirclechart = () => {
                            if (previousIndex <= 3 && activeIndex === 0) circlechartClone.dataset.percentage = "0"; else if (previousIndex > 3 && activeIndex === 0) circlechartClone.dataset.percentage = "100";
                        };
                        document.querySelector(".steps__fraction").textContent = `${this.activeIndex + 1}`;
                        activeIndex === 0 ? circlechartClone.dataset.percentage = "100" : activeIndex === 1 ? circlechartClone.dataset.percentage = "15" : activeIndex === 2 ? circlechartClone.dataset.percentage = "35" : activeIndex === 3 ? circlechartClone.dataset.percentage = "50" : activeIndex === 4 ? circlechartClone.dataset.percentage = "65" : activeIndex === 5 ? circlechartClone.dataset.percentage = "85" : null;
                        document.querySelector(".steps__slider-control").removeChild(circlechart);
                        document.querySelector(".steps__slider-control").appendChild(circlechartClone);
                        $((function() {
        ...
        function headerScroll() {
            ...
            let timer;
            if (scrollTop > scrollDirection) if (!header.classList.contains("_header-show")) {
